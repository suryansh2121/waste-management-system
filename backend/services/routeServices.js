...
exports.getOptimizedRoute = async (dustbinIds) => {
  try {
    ...
    const graph = dustbins.map(d1 =>
      dustbins.map(d2 => ({
        to: d2.id, 
        weight: Math.sqrt(Math.pow(d1.latitude - d2.latitude, 2) + Math.pow(d1.longitude - d2.longitude, 2)) * (1 + d2.fill_level / 100),
      }))
    );

    const routeData = { dustbinIds, dustbins, graph };
    const dijkstraBinary = process.platform === 'win32' ? 'dijkstra.exe' : 'dijkstra';
    const dijkstraPath = path.join(process.cwd(), 'bin', dijkstraBinary);
    const { stdout } = await execFilePromise(dijkstraPath, [JSON.stringify(routeData)], { cwd: process.cwd() });

    let route;
    try {
      route = JSON.parse(stdout);
      if (!Array.isArray(route)) throw new Error('Invalid route output from dijkstra.exe');
    } catch (e) {
